import { SuiNetwork, SuiBaseProcessor, SuiContext } from '@sentio/sdk/sui';
...
for (const address of CONTRACT_ADDRESSES) {
  ...
  SuiBaseProcessor.bind({ network: SuiNetwork.MAIN_NET })
    .onEvent(`${address}::staked_sui_vault::StakedEvent`, async (event: StakedEvent, ctx: SuiContext) => {
    ...
    .onEvent(`${address}::staked_sui_vault::UnstakeRequestedEvent`, async (event: UnstakeRequestedEvent, ctx: SuiContext) => {
